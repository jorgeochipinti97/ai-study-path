# ai-study-path

Personal ML/AI study roadmap. Each book gets a summary and an exam in Spanish and English.

---

## Bloque 0 — Foundations

| # | Book | ES | EN | Exam ES | Exam EN |
|---|------|:--:|:--:|:-------:|:-------:|
| 1 | Machine Learning for Absolute Beginners | [✓](estudio/00-foundations/01-ml-absolute-beginners/resumen-es.md) | [✓](estudio/00-foundations/01-ml-absolute-beginners/resumen-en.md) | [ ] | [ ] |
| 2 | ML Math | [ ] | [ ] | [ ] | [ ] |
| 3 | Hands-On ML with Scikit-Learn, Keras and TensorFlow | [ ] | [ ] | [ ] | [ ] |

## Bloque 1 — Deep Learning & PyTorch

| # | Book | ES | EN | Exam ES | Exam EN |
|---|------|:--:|:--:|:-------:|:-------:|
| 4 | Hands-On ML with Scikit-Learn and PyTorch | [ ] | [ ] | [ ] | [ ] |
| 5 | Hands-On Machine Learning with PyTorch | [ ] | [ ] | [ ] | [ ] |
| 6 | Deep Learning — Goodfellow | [ ] | [ ] | [ ] | [ ] |

## Bloque 2 — LLMs & Fine-Tuning

| # | Book | ES | EN | Exam ES | Exam EN |
|---|------|:--:|:--:|:-------:|:-------:|
| 7 | NLP with Transformer Models | [ ] | [ ] | [ ] | [ ] |
| 8 | Hands-On Large Language Models — Alammar | [ ] | [ ] | [ ] | [ ] |
| 9 | LLM Engineers Handbook | [ ] | [ ] | [ ] | [ ] |

## Bloque 3 — Platform Engineering

| # | Book | ES | EN | Exam ES | Exam EN |
|---|------|:--:|:--:|:-------:|:-------:|
| 10 | AI Engineering | [ ] | [ ] | [ ] | [ ] |
| 11 | Building ML Powered Applications | [ ] | [ ] | [ ] | [ ] |
| 12 | Designing Machine Learning Systems — Chip Huyen | [ ] | [ ] | [ ] | [ ] |
| 13 | Practical MLOps | [ ] | [ ] | [ ] | [ ] |

## Bloque 4 — Generative AI: Images

| # | Book | ES | EN | Exam ES | Exam EN |
|---|------|:--:|:--:|:-------:|:-------:|
| 14 | Generative Deep Learning | [ ] | [ ] | [ ] | [ ] |
| 15 | Hands-On Generative AI with Transformers and Diffusion Models | [ ] | [ ] | [ ] | [ ] |
| 16 | GANs in Action | [ ] | [ ] | [ ] | [ ] |

---

## References

- Applied ML and AI for Engineers
- Pattern Recognition and ML — Bishop
- ML: A Probabilistic Perspective
- Probabilistic ML Advanced Topics
- Artificial Intelligence: A Modern Approach — Russell & Norvig
